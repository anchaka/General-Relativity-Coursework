...
def calculate_a(r_a, v_a,r_norm, M, p):
    ...
    if p == 'no PN':
       ...
       return (- (G * M) * r_a) / (r_norm**3)
        ...
        A_3 = (-8/5) * (G * M * nu * r_dot/ r_norm ) * ((17 / 3) * G * M / r_norm) + (3 * v_norm**2)
        ...
        # Create a dictionary with all the PN correction combinations
        PN_A = {'PN1': A_0 + (c_value**-2*A_1), 
                'PN1 + PN2': A_0 + (c_value**-2*A_1) + (c_value**-4*A_2),
                'PN1 + PN2.5': A_0 + (c_value**-2*A_1) + (c_value**-5*A_3),
                'PN2': A_0 + (c_value**-4*A_2),
                'PN2 + PN2.5': A_0 + (c_value**-4*A_2) + (c_value**-5*A_3),
                'PN2.5': A_0 +(c_value**-5*A_3)}
        
        PN_B = {'PN1': B_0 + (c_value**-2*B_1), 
                'PN1 + PN2': B_0 + (c_value**-2*B_1) + (c_value**-4*B_2),
                'PN1 + PN2.5': B_0 + (c_value**-2*B_1) + (c_value**-5*B_3),
                'PN2': B_0 + (c_value**-4*B_2),
                'PN2 + PN2.5': B_0 + (c_value**-4*B_2) + (c_value**-5*B_3),
                'PN2.5': B_0 +(c_value**-5*B_3)}
        
        # Select only the requested PN correction terms based on p (user input)
        # set other terms to zero
        for key in PN_A:
            if key != 'p':
             PN_A[key] = 0
        A = PN_A['p'] 
...
def variable_leapfrog_algorithm(r_a, v_a, time_period, M, M1, M2, eta, p, color):
    '''
    ...
    a = calculate_a(r_a, v_a, r_norm, M, p)
    ...
    plt.title(f'Orbital Position  of COM of the Binary Blackhole System with {p} correction for a time period {time_period}')
    plt.figure(1)
    ...
    plt.title(f'Eccentricity Evolution of COM of the Binary blackhole System with {p} for a time period {time_period}')
    plt.figure(2)
    plt.plot(t, np.linalg.norm(eccentricity), 'go', markersize=3) 
    plt.xlabel("Time in (years)")
    plt.ylabel("eccentricity of binary blackholes")
    
    # plot numerical evolution of semi-major axis w.r.t. time
    plt.title(f'Semi-Major axis evolution of COM of the Binary Blackhole System with {p} correction for a time period {time_period}')
    plt.figure(3)
    ...
    if p == 'no PN' or p == 'PN1 + PN2':
         variable_leapfrog_algorithm(r_a, v_a, t_p, M, M1, M2, eta, p, color)
    
###########################################################################################################################################
    
def plot_variable_leapfrog_algorithm(r_a, v_a,time_period, M, M1, M2, a, eta, p, color):
     
    variable_leapfrog_algorithm(r_a, v_a, time_period, M, M1, M2, eta, p, color)
...
plot_variable_leapfrog_algorithm(r_a_blackhole, v_a_blackhole, time_period, M_blackhole_binary, blackhole_M1, blackhole_M2, a_blackhole, eta, 'no PN', 'bo')

# Plot Orbit of eccentric black hole binary with PN1 and PN2 correction for 1 orbital period
plot_variable_leapfrog_algorithm(r_a_blackhole, v_a_blackhole, time_period, M_blackhole_binary, blackhole_M1, blackhole_M2, a_blackhole, eta, 'PN1 + PN2', 'yo')

# Display in same plot

# Plot the orbit for a time period of 400 years with PN2.5 correction
time_period = 400
variable_leapfrog_algorithm(r_a_blackhole, v_a_blackhole, time_period, M_blackhole_binary, blackhole_M1, blackhole_M2, eta, 'PN2.5', 'ko')